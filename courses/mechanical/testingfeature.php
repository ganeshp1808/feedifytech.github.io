...
<head><?php getHeader(); ?>
	<style type="text/css">
		...
		#notebookModal .modal-body{ font-family: 'Coming Soon', cursive; background-image:url("images/blankpage.jpg"); background-size: cover; width:100%; height:100%; }
	</style>
</head>
	...
	<section id="contentsection">
		<div class="container">
			...
			<div ng-show="enterData">
				...
				<div class="text-center mb-5" ng-if="((cotterData.skw3mSQ>0) && (cotterData.jsLams2>0))">
				    ...
				    <h1 class="mt-5 mb-4 contentheader"><b>Design File</b></h1><p class="mt-3 mb-3">(Click on it for downloading)</p><h3>.igs / .iges files</h3><br /><div class="row"><div class="col-4"><a class="btn design_content" href="design/cotterjoint/cotter.igs" download>Cotter Design</a></div><div class="col-4"><a class="btn design_content" href="design/cotterjoint/socket.igs" download>Socket Design</a></div><div class="col-4"><a class="btn design_content" href="design/cotterjoint/spigot.igs" download>Spigot Design</a></div></div><br /><br /><br />
				</div>
			</div>
		</div>
		<div class="modal fade" id="notebookModal" tabindex="-1" aria-labelledby="notebookModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                ...
                <?php if(file_exists("files/cotterjoint.txt")==1){ ?>
                    <div class="modal-header"><h5 class="modal-title" id="notebookModalLabel">Your NoteBook (Notes written : <?php echo filesize("files/cotterjoint.txt"); ?>B / 10MB )</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
                    <div class="modal-body ratio ratio-21x9 container"><span class="p-4"><?php $fico = fopen("files/cotterjoint.txt","r"); while(!feof($fico)){ echo fgets($fico). "<br />"; } fclose($fico); ?></span></div>
                <?php }else{ ?>
                	<div class="modal-header"><h5 class="modal-title" id="notebookModalLabel">Note someting</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
                    <div class="modal-body ratio ratio-21x9 container"><span class="p-4">Incomplete</span></div>
                <?php } ?>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
 var myApp=angular.module("machinedesign",[]);
    ...
    myApp.controller("myCotterDesign",function ($scope,$http,$window) { $scope.enterData=true;
        $scope.submitCotter=function(){ $http({  method:"POST", url:"asset/simplejoint/testcotterjoint?token=<?php echo $_SESSION['_token'] ?>", data:$scope.cotterData }).then(function (response){ var data = response.data; if(data.error){ } else{ $scope.ajNzm5r=data.para.ajNzm5r; 
                	$scope.pamzEg4=data.para.pamzEg4; $scope.Jakm2li=data.para.Jakm2li; $scope.kmZw7er=data.para.kmZw7er; } }); };
        $scope.submitNote=function(){ $http({  method:"POST", url:"files/simplejoint/notecotterjoint?token=<?php echo $_SESSION['_token'] ?>", data:$scope.noteData }).then(function (response){ var data = response.data; if(data){ $scope.noteMessage=true;$scope.sNmx=data; } }); };
    });
</script>